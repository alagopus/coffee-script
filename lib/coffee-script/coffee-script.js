...
(function() {
  ...
  exports.run = (function(){ function _run(code, options) {
    ...
    if (!((options.filename && (_ref = file.extension(options.filename), __indexOf.call(extensions, _ref) < 0)) || require.extensions)) {
      code = compile(code, options);
    }
    return compileJS(mainModule, code, mainModule.__filename);
  }; return _run})();
