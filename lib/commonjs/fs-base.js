...
exports.Permissions.prototype.update = function (permissions) {
    ...
    if (typeof permissions == "number") {
        var off = exports.UNIX_BITS.length - 9;
        var bits = permissions.toString(2);
        if (bits.length < 9) {
            bits = "000000000".substring(bits.length) + bits;
        }
        for (var i = 9; --i >= 0;) {
            var upp = exports.UNIX_BITS[off + i];
            var bit = bits.charAt(i);
            this.grant(upp[0], upp[1], bit === "1");
        }
    } else for (var user in permissions) {
        function has(o, k) { return o.hasOwnProperty(k); }
        if (has(permissions, user)) {
            this[user] = this[user] || {};
            for (var permission in permissions[user]) {
                if (has(permissions[user], permission)) {
...
exports.changeWorkingDirectory = function (path) {
    path = String(path);
    var error = getCLib().getFunction(Packages.com.sun.jna.Platform.isWindows() ? "_chdir" : "chdir").invokeInt([path]);
    if (error)
    ...
    var jna = Packages.com.sun.jna;
    var cwd = getCLib().getFunction(Packages.com.sun.jna.Platform.isWindows() ? "_getcwd" : "getcwd");
